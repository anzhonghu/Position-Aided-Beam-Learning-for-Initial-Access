...
Ntheta = [-16;-12;-8;-4;0];
access_delay = zeros(2, length(Ntheta));%2 methods
misdetection = zeros(5, length(Ntheta));%5 methods
misdetbound = zeros(5, 1);
ahievrate = zeros(5, length(Ntheta));%5 methods
Tsetnumber = 100;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
for nnids = 1 : length(Ntheta)
    Thetathresdb = Ntheta(nnids,1);
    succcount = zeros(5,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%
semilogy(Ntheta, misdetection(1, :), 'r--^','LineWidth',1,'MarkerSize',10)
hold on
semilogy(Ntheta, misdetection(2, :), 'r--v','LineWidth',1,'MarkerSize',10)
semilogy(Ntheta, misdetection(3, :), 'g--*','LineWidth',1,'MarkerSize',10)
semilogy(Ntheta, misdetection(5, :), 'b--x','LineWidth',1,'MarkerSize',10)
semilogy(Ntheta, misdetection(4, :), 'k-o','LineWidth',1,'MarkerSize',10)
semilogy(Ntheta, misdetbound, 'k-s','LineWidth',1,'MarkerSize',10)
le = legend('PI [4]', 'PI [19]', 'Exhaustive [5]', 'SVM [29][30]', 'Proposed','Bound (17)',  'Location', 'southeast');
set(le,'Fontname','Times')
set(gca,'XTick',Ntheta)
xlabel('Threshold (dBm)','Fontname','Times')
