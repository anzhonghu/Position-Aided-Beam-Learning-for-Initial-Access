...
access_delay = zeros(2, length(Ncls));%2 methods
misdetection = zeros(5, length(Ncls));%5 methods
misdetbound = zeros(5, 1);
ahievrate = zeros(5, length(Ncls));%5 methods
Tsetnumber = 100;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
for nnids = 1 : length(Ncls)
    Ncl = Ncls(nnids,1);
    succcount = zeros(5,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%
semilogy(Ncls, misdetection(1, :), 'r--^','LineWidth',1,'MarkerSize',10)
hold on
semilogy(Ncls, misdetection(2, :), 'r--v','LineWidth',1,'MarkerSize',10)
semilogy(Ncls, misdetection(3, :), 'g--*','LineWidth',1,'MarkerSize',10)
semilogy(Ncls, misdetection(5, :), 'b--x','LineWidth',1,'MarkerSize',10)
semilogy(Ncls, misdetection(4, :), 'k-o','LineWidth',1,'MarkerSize',10)
semilogy(Ncls, misdetbound, 'k-s','LineWidth',1,'MarkerSize',10)
le = legend('PI [4]', 'PI [19]', 'Exhaustive [5]', 'SVM [29][30]', 'Proposed','Bound (17)',  'Location', 'southeast');
set(le,'Fontname','Times')
set(gca,'XTick',Ncls)
xlabel('Number of clusters N_{cl}','Fontname','Times')
