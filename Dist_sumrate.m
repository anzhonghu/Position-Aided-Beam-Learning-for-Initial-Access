...
Ndis = [60;70;80;90;100];
access_delay = zeros(2, length(Ndis));%2 methods
misdetection = zeros(5, length(Ndis));%5 methods
misdetbound = zeros(5, 1);
ahievrate = zeros(5, length(Ndis));%5 methods
Tsetnumber = 100;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
for nnids = 1 : length(Ndis)
    Rmax = Ndis(nnids, 1);
    Rmid = (Rmax+Rmin) * 0.5;
    succcount = zeros(5,1);
...
ahievrate = ahievrate / (Ttotal - Tsetnumber);
%%%%%%%%%%%%%%%%%%%%%%%%%
semilogy(Ndis, ahievrate(1, :), 'r--^','LineWidth',1,'MarkerSize',10)
hold on
semilogy(Ndis, ahievrate(2, :), 'r--v','LineWidth',1,'MarkerSize',10)
semilogy(Ndis, ahievrate(3, :), 'g--*','LineWidth',1,'MarkerSize',10)
semilogy(Ndis, ahievrate(5, :), 'b--x','LineWidth',1,'MarkerSize',10)
semilogy(Ndis, ahievrate(4, :), 'k-o','LineWidth',1,'MarkerSize',10)
le = legend('PI [4]', 'PI [19]', 'Exhaustive [5]', 'SVM [29][30]', 'Proposed',  'Location', 'southeast');
set(le,'Fontname','Times')
set(gca,'XTick',Ndis)
xlabel('Outer radius (m)','Fontname','Times')
ylabel('Spectral efficiency (bps/Hz)','Fontname','Times')
grid on%%%%%%%%%%%%%%%%%%%%%%%%
